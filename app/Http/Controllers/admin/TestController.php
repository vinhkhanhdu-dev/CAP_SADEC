...
class TestController extends Controller
{
    public function index(Request $request)
    {
        try {
            ...
            $queryService = new QueryService(new test());
    ...
    public function update(Request $request, $id)
    {
        try {
            $formData = $request->post();
            $res = test::find($id)->update($formData);
    ...
    public function genCode(){
        $lastCode = test::orderBy('code', 'desc')->first(); // lấy mã cuối cùng trong database      
        if (!$lastCode) {
            $number = 1;
        } else {
            $number = intval(substr($lastCode->code, -3)) + 1; // lấy số cuối cùng của mã và tăng giá trị lên 1
        }    
        $newCode = 'CD' . str_pad($number, 4, '0', STR_PAD_LEFT); // tạo mã mới dựa trên số đó và định dạng "ABCXXX"
        return $newCode;
    }
}